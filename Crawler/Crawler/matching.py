...
def transform(text):
    ...
    screentech = ["screentech", "màn hình", "công nghệ màn hình", "Công nghệ màn hình"]
    screensize = ["screensize", "kích thước màn hình", "Màn hình rộng", "màn hình rộng"]
    cpu = ["cpu", "chip", "chipset", "Bộ vi xử lý", "Cấu hình", "cấu hình", "Chipset (hãng SX CPU)", "chipset (hãng sx cpu)"]
    rom = ["rom", "bộ nhớ trong", "Ổ cứng", "Memory Storage Capacity", "Bộ nhớ trong"]
    ram = ["ram", "Ram"]
    pin = ["pin", "PIN", "dung lượng pin", "Batteries"]
    gpu = ["gpu", "đồ hoạ", "vga", "card hình", "Chip đồ họa (GPU)", "chip đồ họa (gpu)"]
