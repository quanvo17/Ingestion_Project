...
class iphone_viettable(scrapy.Spider):
    name = 'iphone_viettable'
    start_urls = ["https://www.viettablet.com/iphone"]
    script = """
    ...
    def parse(self, response):
        ...
        for item in items:
            ...
            informations = soup.find_all('li', class_='features-list-item')
            for information in informations:
                label = str(information.text).split(': ')[0].replace('\t', '').replace('\n', '')
                value = str(information.text).split(': ')[1].replace('\t', '').replace('\n', '')
